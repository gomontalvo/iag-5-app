...
from db import db
from models import User, Preferencias
...
def where_to_watch(client: OpenAI, search_term: str, user: User):
    ...
    if not movie_or_tv_show:
        return f'No estoy seguro de dónde puedes ver esta película o serie :(, pero quizas puedes revisar en TMDB: https://www.themoviedb.org/search?query={search_term}'
...
def search_movie_creditos(client: OpenAI, search_term: str, user: User):
    creditos = search_creditos(search_term)

    if not creditos:
        return f'No estoy seguro si el nombre de la pelicula o serie :(, pero quizas puedes revisar en TMDB: https://www.themoviedb.org/search?query={search_term}'
